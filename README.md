...
with a dot in the attachment name, the package itself is unaffected:
    ...
    sudo dpkg -i dynotiq_0.5.beta_all.deb

Build the package yourself:

    ./build-deb.sh
    sudo dpkg -i build/dynotiq_0.6~beta_all.deb
...
`0.6~beta~ubuntu24.04.1` sorts below `0.6~beta~ubuntu26.04.1`. Series names
cycle back through the alphabet, which would eventually make a newer Ubuntu look
older to apt. A tilde sorts below everything, which is what makes `0.6~beta`
an upgrade from `0.5` and still a downgrade from the final `0.6`. Git refuses a
tilde in tag names, so the matching tag is written `v0.6-beta`.
